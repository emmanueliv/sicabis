 ...
 * @author fabianvr
 */
@ManagedBean(name = "consultaRescisionesBean")
@ViewScoped
public class ConsultaRescisionBean {
    ...
    private List<Rescisiones> detalleList;
    ...
    public ConsultaRescisionBean() {
        penas = FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().get("idRescision");
    ...
    public void rescisiones() {
        idDetalle = Integer.parseInt(String.valueOf(penas));
        detalleList = rescisionesService.rescisiones(idDetalle);
    ...
    public void consultaRescisiones() {
        verRemision = false;
        for (Rescisiones dos : detalleList) {
    ...
    public void verRescision() {
        ...
        if (res != null) {
            for (Rescisiones r : res) {
                if (r.getIdEstatus().getIdEstatus() == 155) {
                    verInicioR = true;
                    verCancelado = false;
                    verInicio = false;
                    verInicioRescision = "true";
                } else if (r.getIdEstatus().getIdEstatus() == 156) {
                    verRescision = true;
                    verCancelado = false;
                    verInicio = false;
                    verInicioRescision = "true";
